...
def loss_for_val(X_proc, X_raw, generator_model, e):
    ...
    sum_l1 = 0
    sum_rmse = 0
    height, width = X_gen[0].shape[:2]
    size = height * width
    for i in range(len(X_gen)):
        diff = np.abs(X_gen[i] - X_proc[i])
        sum_l1 += (np.sum(diff) / size)  
        sum_rmse += math.sqrt(np.sum(np.square(diff)) / size)

    l1_value = sum_l1 / X_gen.shape[0]
    rmse_value = sum_rmse / len(X_gen)
    
    # chack shape of result
    print(X_gen.shape)
    Xs = to3d(X_raw[:5])
    Xg = to3d(X_gen[:5])
    Xr = to3d(X_proc[:5])
    ...
    plt.imshow(XX)
    plt.axis('off')
    plt.savefig("./figures/current_batch" + str(e) + ".png")
    plt.clf()
    plt.close()

    return l1_value, rmse_value
...
def plot_generated_batch(X_proc, X_raw, generator_model, batch_size, suffix, current_ephoch):
    X_gen = generator_model.predict(X_raw)
    ...
    plt.imshow(XX)
    plt.axis('off')
    plt.savefig("./figures/current_batch_" + suffix + "_" + str(current_ephoch) + ".png")
    plt.clf()
    plt.close()
...
def my_train(args):
    ...
    for e in range(args.epoch):
        ...
        for (X_proc_batch, X_raw_batch) in zip(X_procImageIter, X_rawImageIter):
            ...
            progbar.add(args.batch_size, values=[("D logloss", disc_loss),
                ("G tot", gen_loss[0]),
                ("G L1", gen_loss[1]),
        ...
        l1, rmse = loss_for_val(procImage_val, rawImage_val, generator_model, e)
        generator_model.save('./model_weights/' + str(e + 1) + 'epoch_L1_' + str(l1) + '.hdf5', include_optimizer=False)
